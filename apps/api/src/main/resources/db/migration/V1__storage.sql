-- Flyway V1: PostgreSQL 17 storage (faithful copy of spec/v1/storage.sql).
-- One version = one transaction on PostgreSQL (Flyway default). Do not split unless FK order requires it.
-- PG-specific: GENERATED ALWAYS AS IDENTITY, functional index lower(technical_name). Not for H2.
